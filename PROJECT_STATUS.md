...
- 修复转写页选择本地音频时调用已删除的 `formatAsrTime` 导致渲染中断；独立时长格式显示，并为无文字正文的已完成结果显示明确提示。历史任务点击恢复正文显示，当前任务选中态加强。
- MiMo 任务阶段与真实分片处理同步：准备音频、片段开始/完成、整理文字稿；完成全部片段后才进入 100%，不再长时间停在固定 40%。

## 已通过的测试

- `python -m py_compile app.py`、`node --check static/app.js`、`node --check static/asr.js`、`git diff --check`。
- 本轮真实浏览器回归：本机持久化 `mimo-v2.5-asr` 成功稿无需重转写即显示（13,915 字符）；选择合成 2 秒 WAV 后正确显示 `0:02`，无 `formatAsrTime` ReferenceError。隔离任务页面验证 A/B 点击显示各自正文，C 状态轮询不切走 A；浏览器无页面异常。
- 本轮后端模拟 3 个真实分片事件（未请求云端）：v2.5 与 Flash 均依次产生准备 10%、就绪 15%、每片开始/完成（20%→90%）和整理 95%；结果按序合并，最终任务完成逻辑仍设置 100%。
